...
#    Use RELATIVE baseurlpath and export SIMPLESAMLPHP_CONFIG_DIR immediately.
# --------------------------------------------------------------------------------------
...
# -- config.php: explicit flatfile metadata directory + RELATIVE baseurlpath
cat > "$SSP_TEMP_CONFIG_DIR/config.php" <<'PHP'
<?php
$config = [
    // RELATIVE path; combined with current host -> http://example.org/simplesaml/
...
# --------------------------------------------------------------------------------------
# (Now the env var is exported, this works)
