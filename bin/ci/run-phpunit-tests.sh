...
// 1. Load Composer autoloader to get access to the Configuration class.
require_once dirname( __DIR__, 2 ) . '/vendor/autoload.php';

// 2. **DEFINITIVE FIX**: Forcefully tell SimpleSAMLphp where to find its config.
//    This bypasses all auto-detection logic.
\SimpleSAML\Configuration::setConfigDir('/tmp/simplesamlphp-config');

// 3. Load the SimpleSAMLphp autoloader.
...
// 4. Load WordPress test environment.
...
# 7. Run the tests
# The SIMPLESAMLPHP_CONFIG_DIR environment variable is NO LONGER NEEDED,
# because the path is now hardcoded directly in the bootstrap file.
composer phpunit