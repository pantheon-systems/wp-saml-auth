...
###
# Test WP SAML Auth with SimpleSAMLphp 2.3.7 (recommended version)
# This script installs SimpleSAMLphp 2.3.7 and tests the security warnings and authentication
###

set -ex

# Define the SimpleSAMLphp version to test
SIMPLESAMLPHP_VERSION="2.3.7"
echo "Testing with SimpleSAMLphp version $SIMPLESAMLPHP_VERSION (recommended version)"
...
echo "- No warnings in admin"
echo "- Authentication allowed"
echo "- Settings page shows no warnings"
echo ""
echo "Please check the admin interface at http://localhost/wp-admin/ to verify no warnings are shown"
